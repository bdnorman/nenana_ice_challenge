...
def main():
    ...
    mse = mean_squared_error(
        df_val["Decimal Day of Year"].values[num_lags - 1 :],
        y_preds[num_lags - 1 :].values,
    )
    ...
    plt.plot(
        df_val["Year"].values[(num_lags - 1) :],
        df_val["Decimal Day of Year"].values[num_lags - 1 :],
        "b",
    )
    plt.plot(
        df_val["Year"].values[(num_lags - 1) :], y_preds[num_lags - 1 :].values, "r"
    )
    plt.legend(
        [
            "Actual Melt",
